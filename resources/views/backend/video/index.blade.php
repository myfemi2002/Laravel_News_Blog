...
<div class="container-fluid">
    <div class="col-lg-12">
        <div class="card">
            <div class="header">
                <h2>@yield('title')</h2>
                <a href="{{ route('video.add') }}" class="btn btn-rounded btn-success" style="float: right;"><i class="fa fa-plus-circle"></i> Add </a>
            </div>
            <div class="body table-responsive">
                <table class="table table-bordered mb-0">
                    ...
                    <tbody>
                        @foreach ($video as $key => $rows)
                        <tr>
                            <td>{{ $key+1 }}</td>
                            <td> {{ $rows->title }} </td>
                            <td> {{ Str::limit($rows->embed_code, 20) }} </td>
                            <td>
                                @if($rows->type == 1)
                            ...
                            <td>
                                <a href="{{ route('video.edit',$rows->id) }}" class="btn btn-primary btn-rounded btn-sm text-white" title="Edit Data" > <i class="fa fa-edit"></i></a>
                                <a href="{{ route('video.delete',$rows->id) }}"  class="btn btn-danger btn-rounded btn-sm"  id="delete" title="Delete Data" > <i class="fa fa-trash"></i></a>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
